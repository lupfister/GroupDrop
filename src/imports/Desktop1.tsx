...
export default function Desktop() {
  ...
  const updatePotentialGroups = () => {
    ...
    // Find all phones within proximity of each other (excluding those in confirmed groups and recently removed)
    const activeBodies = bodiesRef.current.filter(body => 
      !phonesInConfirmedGroups.has(body.id) && !recentlyRemovedPhonesRef.current.has(body.id)
    );
    ...
    for (const body of activeBodies) {
      if (!visited.has(body.id) && proximityMap.has(body.id)) {
        ...
        if (filteredComponent.size >= 2) {
          ...
          setPotentialGroups(prev => {
            const next = new Map(prev);
            // Ensure we don't include any recently removed phones - double check as safety measure
            const cleanMemberIds = new Set(
              Array.from(filteredComponent).filter(id => 
                !recentlyRemovedPhonesRef.current.has(id) && 
                activeBodies.some(b => b.id === id) // Also verify they're in activeBodies
              )
            );
            
            // Don't create/update group if it would contain recently removed phones or have less than 2 members
            if (cleanMemberIds.size < 2) {
              return prev; // Don't create/update the group
            }
            
            const cleanConfirmedIds = existingGroup 
              ? new Set(
                  Array.from(existingGroup.confirmedIds).filter(id => 
                    !recentlyRemovedPhonesRef.current.has(id) && 
                    cleanMemberIds.has(id) // Only keep confirmed IDs that are still in the group
                  )
                )
              : new Set();
    ...
    setPotentialGroups(prev => {
      ...
      next.forEach((group, groupId) => {
        ...
        const membersWithoutRemoved = Array.from(group.memberIds).filter(id => 
          !recentlyRemovedPhonesRef.current.has(id) && allActiveIds.has(id)
        );
        
        const hasActiveMembers = membersWithoutRemoved.length > 0;
        if (!hasActiveMembers) {
          next.delete(groupId);
        } else {
          // Update memberIds to match current proximity, excluding recently removed
          const currentMembers = membersWithoutRemoved;
          if (currentMembers.length < 2) {
            next.delete(groupId);
          } else {
            // Update the group to remove recently removed phones
            const newMemberIds = new Set(currentMembers);
              ...
              <div>
                ...
                {recentlyRemovedPhones.size === 0 ? (
                  ...
                  <div className="bg-red-50 border border-red-300 rounded-md p-3">
                    <div className="text-sm text-gray-700">
                      <span className="font-medium">Removed IDs:</span> {Array.from(recentlyRemovedPhones).join(', ')}
                    </div>
                  </div>
                )}
              </div>
